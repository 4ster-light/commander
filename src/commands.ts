import { blue, bold, cyan, green, underline, yellow } from "@std/fmt/colors"
...
export function showHelp() {
  ...
  console.log(`    ${yellow("-c")}, ${yellow("--config=<path>")}     Specify config file path (default: commander.toml)`)
  ...
  console.log(`    ${blue("commander")} ${green("scripts")}`)
  console.log(`    ${blue("commander")} ${yellow("--init")}`)
  console.log(`    ${blue("commander")} ${yellow("--dir=scripts")}`)
  console.log(`    ${blue("commander")} ${yellow("--config=custom.toml")}`)
}
